...
<head>
    ...
    <title>@yield('title', 'Admin') | MarocLoi {{ auth()->user()->isAdmin() ? 'Admin' : 'Advisor Portal' }}</title>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@100..900&family=Playfair+Display:ital,wght@0,400..900;1,400..900&display=swap" rel="stylesheet" media="print" onload="this.media='all'">
...
<div x-data="{ sidebarOpen: false }" @keydown.escape.window="sidebarOpen = false" class="min-h-screen lg:flex">
        ...
        <div>
            ...
            <div class="px-6 py-3.5 border-b border-slate-200 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs font-medium text-slate-500 mt-0.5">{{ auth()->user()->isAdmin() ? 'Administration Portal' : 'Advisor Portal' }}</p>
                </div>
            ...
            <nav class="p-4 space-y-1">
                @if(auth()->user()->isAdmin())
                    ...
                    <a href="{{ route('admin.orders.index') }}"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.orders*') ? 'bg-blue-600 text-white font-semibold shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                        Orders
                    </a>

                    <a href="{{ route('admin.advisors.index') }}"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('admin.advisors*') ? 'bg-blue-600 text-white font-semibold shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                        Advisors
                    </a>
                @else
                    <a href="{{ route('advisor.dashboard') }}"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('advisor.dashboard') ? 'bg-blue-600 text-white font-semibold shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 00-1-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 00-1 1m-6 0h6"/></svg>
                        Dashboard
                    </a>

                    <a href="{{ route('advisor.cases.index') }}"
                       class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition {{ request()->routeIs('advisor.cases*') ? 'bg-blue-600 text-white font-semibold shadow-sm' : 'text-slate-500 hover:bg-slate-200 hover:text-slate-900' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                        Advisor Cases
                    </a>
                @endif
    ...
    <main class="flex-1 flex flex-col min-w-0 lg:pl-64">
                ...
                <div class="flex items-center gap-3 shrink-0">
                    <a href="{{ route('landing') }}"
                       class="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-slate-700 text-xs font-semibold transition hover:bg-slate-100 shadow-sm">
                        ...
                        <span>{{ __('landing.back_to_landing_page') }}</span>
                    </a>
                    ...
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open"
                            ...
                            <a href="{{ route('app.workspace') }}" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50 font-medium">
                                ...
                                {{ __('landing.law_search_workspace') }}
                            </a>
        ...
        @if(session('error'))
            <div class="mx-4 lg:mx-8 mt-6 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-sm flex items-center gap-2">

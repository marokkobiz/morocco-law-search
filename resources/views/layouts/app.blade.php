...
<head>
    ...
    <!-- Dark Mode Init Script (Prevents Screen Flickering) -->
    <script>
...
<body class="font-sans antialiased text-gray-900 dark:text-gray-100 bg-white dark:bg-slate-900 transition-colors duration-200 {{ $isArabic ? 'rtl' : '' }}">
    <header class="sticky top-0 z-40 bg-gray-900 border-b border-gray-800">
        <div class="flex items-center h-14 px-4 lg:px-6 gap-4">
            <a href="/" class="flex items-center gap-2.5 shrink-0 no-underline">
                ...
                <span class="text-sm font-bold text-white hidden sm:inline">Maroc<span class="text-blue-400">Loi.com</span></span>
            </a>
            ...
            <div class="flex items-center gap-3 shrink-0">
                ...
                <button onclick="toggleDarkMode()" 
                        class="p-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300 border border-gray-700 transition-colors focus:outline-none cursor-pointer" 
                    ...
                    <svg class="w-4 h-4 block dark:hidden text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <details class="relative">
                    <summary class="list-none cursor-pointer rounded-lg border border-gray-700 bg-gray-800 px-3 py-1.5 text-xs font-bold text-gray-300 transition-colors hover:border-gray-600 hover:text-white [&::-webkit-details-marker]:hidden">
                ...
                @auth
                    <a href="{{ route('app.workspace') }}" class="h-8 flex items-center px-4 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-lg transition-colors no-underline">{{ $locale === 'fr' ? 'Dashboard' : ($locale === 'ar' ? 'لوحة التحكم' : 'Dashboard') }}</a>
                @else
                    <a href="{{ route('login') }}" class="inline-flex items-center text-xs h-8 px-4 py-1.5 rounded-lg font-semibold text-white border border-white/30 hover:bg-white/10 transition-all duration-200 no-underline">{{ $layoutCopy['login'] }}</a>
            ...
            <div class="flex items-center gap-1 sm:gap-2 shrink-0">
                @auth
                    <details class="relative">
                        <summary class="list-none cursor-pointer w-8 h-8 rounded-md bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-300 hover:bg-gray-600 transition-colors [&::-webkit-details-marker]:hidden">
                        ...
                        <div class="absolute ltr:right-0 rtl:left-0 z-50 mt-2 w-52 overflow-hidden rounded-xl border border-gray-200 bg-white py-2 shadow-xl">
                            ...
                            <div class="lg:hidden px-4 py-2 border-b border-gray-100">
                                ...
                                <div class="flex gap-1">
                                    @foreach ($localeOptions as $localeOption => $localeMeta)
                                        <a href="{{ route('locale.switch', $localeOption) }}" class="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-xs font-semibold no-underline transition-colors {{ $activeLocale === $localeOption ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' : 'text-gray-500 hover:bg-gray-100' }}">
                                            <span>{{ $localeMeta['code'] }}</span>
                                        </a>
                                    @endforeach
                                </div>
                            </div>
                            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors no-underline">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div>
                    ...
                    <div class="text-center">
                        <div class="flex flex-wrap items-center justify-center gap-x-1 text-[10px] sm:text-xs text-slate-400">
                            <a href="https://www.de-bail.com" target="_blank" rel="noopener noreferrer" class="hover:text-slate-500 transition-colors duration-200 no-underline">de-bail.com</a>
                            <span class="select-none"> - </span>
                            <a href="https://www.marokkobiz.com" target="_blank" rel="noopener noreferrer" class="hover:text-slate-500 transition-colors duration-200 no-underline">marokkobiz.com</a>
                            <span class="select-none"> - </span>
                            <a href="https://www.marokkobiztv.com" target="_blank" rel="noopener noreferrer" class="hover:text-slate-500 transition-colors duration-200 no-underline">marokkobiztv.com</a>
                        </div>
                    </div>
                </div>

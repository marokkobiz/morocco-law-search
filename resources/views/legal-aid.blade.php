                ...
                <div class="card p-8" data-animate="fade-up" style="--delay:.2s">
                    ...
                    <form class="space-y-6" action="{{ route('legal-aid.store') }}" method="POST">
                        ...
                        <div id="consultation-section" class="{{ ! empty(old('service_ids')) ? '' : 'hidden' }}">
                            <input type="hidden" name="consultation_mode" value="whatsapp" id="consultation_mode_hidden">
                            ...
                            <div id="consultation-question">
                                ...
                                <div class="grid gap-2 sm:grid-cols-2">
                                    <label id="mode-office-wrap"
                                        ...
                                        <input type="radio" name="consultation_mode_choice" value="office" class="sr-only"
                                    ...
                                    <label id="mode-whatsapp-wrap"
                                        ...
                                        <input type="radio" name="consultation_mode_choice" value="whatsapp" class="sr-only"
                            ...
                            <div id="consultation-office-only" class="hidden">
                                <div class="flex items-start gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3">
                                    <svg class="mt-0.5 h-5 w-5 shrink-0 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 21h18M4 21V8a2 2 0 012-2h12a2 2 0 012 2v13M9 21v-5h6v5M9 7V5a2 2 0 012-2h2a2 2 0 012 2v2"></path>
                                    </svg>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900">{{ __('legal_aid.consultation_office') }}</p>
                                        <p class="mt-0.5 text-xs text-gray-500">{{ __('legal_aid.consultation_office_only') }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="consultation-whatsapp-only" class="hidden">
                                <div class="flex items-start gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
                                    <svg class="mt-0.5 h-5 w-5 shrink-0 text-green-600" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.52.149-.174.198-.298.297-.497.1-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"></path>
                                    </svg>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-900">{{ __('legal_aid.consultation_whatsapp') }}</p>
                                        <p class="mt-0.5 text-xs text-gray-500">{{ __('legal_aid.consultation_whatsapp_only') }}</p>
                                    </div>
                                </div>
                            </div>
...
@push('scripts')
    <script>
        (function () {
            ...
            const singleBanners = { office: officeOnlyDiv, whatsapp: whatsappOnlyDiv };
            ...
            function updateConsultationModes() {
                ...
                Object.keys(singleBanners).forEach((key) => {
                    if (singleBanners[key]) singleBanners[key].classList.toggle('hidden', !(isSingleMode && key === singleMode));
                });
